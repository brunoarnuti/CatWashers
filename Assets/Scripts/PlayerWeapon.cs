...
public class PlayerWeapon : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            waterPressureManager.Pump(); // Call the Pump method when the spacebar is pressed
        }
    }
    ...
    {
        int numberOfDroplets = 3; // Number of droplets in each shot
        float dispersionAngle = 2f; // Small angle for slight dispersion
        ...
        while (isFiring)
        {
            ...
            // Instantiate multiple water droplets with slight dispersion
            for (int i = 0; i < numberOfDroplets; i++)
            {
                // Calculate the dispersion angle for this droplet
                float angle = UnityEngine.Random.Range(-dispersionAngle, dispersionAngle);

                // Add a random offset to the position
                Vector3 positionOffset = new Vector3(UnityEngine.Random.Range(-0.1f, 0.1f),
                    UnityEngine.Random.Range(-0.1f, 0.1f), 0f);

                // Instantiate the water droplet
                GameObject waterDrop = Instantiate(waterDropPrefab, weaponShootPoint.position + positionOffset,
                    Quaternion.identity);

                // Initialize the water droplet with the direction, current range, and dispersion angle
                waterDrop.GetComponent<WaterDrop>().Initialize(Quaternion.Euler(0, 0, angle) * direction, currentRange);
            }
